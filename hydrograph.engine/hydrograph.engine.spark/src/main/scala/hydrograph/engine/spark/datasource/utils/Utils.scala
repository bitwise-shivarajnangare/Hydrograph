...
import java.util.{Collections, Locale, Properties, Random, UUID}
...
*/
  ...
  /**
    ...
    * set of directories.
    */
  /* private def getOrCreateLocalRootDirs(conf: SparkConf): Array[String] = {
...
*/
  ...
  private def clearLocalRootDirs(): Unit = {
    localRootDirs = null
  }
  ...
  def isBindCollision(exception: Throwable): Boolean = {
    exception match {
      ...
      case e: MultiException => true
      //e.getThrowables.asScala.exists(isBindCollision(e.getCause))
    ...
    if (groupProviderClassName != "") {
      try {
        val groupMappingServiceProvider = classForName(groupProviderClassName).newInstance.
          asInstanceOf[org.apache.hadoop.security.GroupMappingServiceProvider]
        val currentUserGroups = groupMappingServiceProvider.getGroups(username)
        return currentUserGroups.toArray.map(_.toString).toSet
      } catch {
