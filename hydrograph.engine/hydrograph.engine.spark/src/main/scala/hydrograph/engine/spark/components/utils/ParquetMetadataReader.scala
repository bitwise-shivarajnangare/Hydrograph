...
class ParquetMetadataReader(
  ...
  private def convertPrimitiveField(field: PrimitiveType): DataType = {
    ...
    def typeNotSupported() =
      throw new RuntimeException(s"Parquet type not supported: $typeString")

    def typeNotImplemented() =
      throw new RuntimeException(s"Parquet type not yet supported: $typeString")

    def illegalType() =
      throw new RuntimeException(s"Illegal Parquet type: $typeString")
  ...
  private def convertGroupField(field: GroupType): DataType = {
    Option(field.getOriginalType).fold(convert(field): DataType) {
      ...
      case _ =>
        throw new RuntimeException(s"Unrecognized Parquet type: $field")
    }
  }
  ...
  private def convertField(field: StructField, repetition: Type.Repetition): Type = {
    ...
    field.dataType match {
      ...
      //
      // Converts `TIMESTAMP_MICROS` once parquet-mr implements that.
      case TimestampType =>
      ...
      // by `DECIMAL`.
      case Fixed(precision, scale) if writeLegacyParquetFormat =>
        Types
      ...
      case _ =>
        throw new RuntimeException(s"Unsupported data type $field.dataType")
    }
  }
}
